...
/* Define the PWM settings for backlight */
#define PWM_BACKLIGHT_NODE   DT_ALIAS(pwm_dev_0) /* New */
#define BACKLIGHT_CHANNEL    3        /* The PWM channel, adjust based on your hardware */
#define BACKLIGHT_PERIOD     1000000U /* 1 MHz */
#define BACKLIGHT_BRIGHTNESS 500000U  /* 50% duty cycle */

static const struct device *pwm_backlight;
static const struct device *display_dev;

/* Initialize the PWM for the backlight */
static void init_backlight(void)
{
	pwm_backlight = DEVICE_DT_GET(PWM_BACKLIGHT_NODE);
	if (!device_is_ready(pwm_backlight)) {
		LOG_ERR("PWM device not ready");
		return;
	}

	int ret = pwm_set(pwm_backlight, BACKLIGHT_CHANNEL, BACKLIGHT_PERIOD, BACKLIGHT_BRIGHTNESS,
			  0);
	if (ret) {
		LOG_ERR("Failed to set PWM for backlight: %d", ret);
	} else {
		LOG_INF("Backlight initialized");
	}
}

/* Initialize the ILI9163 display */
...
int main(void)
{
	...
	lv_label_set_text(hello_world_label, "Hello Clement!");
